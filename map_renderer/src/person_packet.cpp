...
std::string decodeName(const std::uint8_t *bytes) {
  std::size_t length = 0;
  while (length < kPersonNameBytes && bytes[length] != 0) {
    ++length;
  }
  return std::string(reinterpret_cast<const char *>(bytes), length);
}

void encodeName(std::uint8_t *bytes, const std::string &name) {
  std::memset(bytes, 0, kPersonNameBytes);
  const std::size_t copy =
      std::min(name.size(), static_cast<std::size_t>(kPersonNameBytes - 1));
  if (copy > 0) {
...
std::optional<PersonUpdate> parsePersonPacket(const std::uint8_t *data,
  ...
  if (size != kPersonPacketSize) {
    setError(error, "packet must contain exactly 44 bytes");
    return std::nullopt;
  }
  ...
  update.name = decodeName(data + 28);
...
PersonPacketBytes serializePersonPacket(const PersonUpdate &update) {
  ...
  encodeName(bytes.data() + 28, update.name);
  return bytes;
}
